...
std::optional<Ship> Player::ship_at_position(std::uint16_t row,
...
bool Player::generate_random_ships(Game const &game, ErrorReport &report) {
  ...
  for (uint32_t ship_id = game.minShipSize; ship_id < game.maxShipSize + 1;
       ++ship_id) {
    ...
    for (size_t count_attempts = 0; count_attempts < 1000; ++count_attempts) {
      ...
      std::uint16_t width = (layout == Orientation::Vertical ? 0 : ship_id - 1);
      std::uint16_t height =
          (layout == Orientation::Horizontal ? 0 : ship_id - 1);

      int row = randomns::between(0, game.rows - 1 - height);
      int col = randomns::between(0, game.cols - 1 - width);
    ...
    if (ships.size() == size_vector) {
      report(std::format("Player {} could not generate: {} after {} tries",
                         name, ship_id, 1000));
      return false;
    }
  }
...
bool Game::report_game_is_valid(ErrorReport &report) const noexcept {
  for (auto &player : players) {
    for (auto shipit = player.ships.begin(); shipit != player.ships.end();
         ++shipit) {
      auto &ship = *shipit;
      if (!ship.is_valid()) {
      ...
      if (ship.shiplength > maxShipSize || ship.shiplength < minShipSize) {
        report(std::format(
            "Player: {}'s Ship Id: {} is larger than game specified.",
            player.name, ship.id()));
        return false;
      }

      if (std::ranges::any_of(
              shipit + 1, player.ships.end(), [&](auto &next_ship) {
                /*std::cout << std::format("{}'s comparing {} to {} collides =
                   {}\n", player.name, ship.id(), next_ship.id(),
                   aabb_collision(ship.location, next_ship.location));*/

                if (aabb_collision(ship.location, next_ship.location)) {
                  report(
                      std::format("{}'s ships collide:  {} collides with {}.",
                                  player.name, ship.id(), next_ship.id()));
                  return true;
                }
...
void print_game(Game const &g) {
  std::cout << std::format("Game:\nRows: {}\nCols: {}\nShips: {} to {}\n",
                           g.rows, g.cols, g.minShipSize, g.maxShipSize);
  for (auto &p : g.players) {
    ...
    for (auto &s : p.ships) {
      ...
      std::cout << '\t' << "Ship orientation: "
                << (s.orientation() == Orientation::Horizontal ? 'H' : 'V')
                << '\n';
      std::cout << '\t'
                << std::format("Ship Location: x: {} y: {} x2: {} y2: {}",
                               s.location.x, s.location.y, s.location.x2,
                               s.location.y2)
                << '\n';
    }
  }
