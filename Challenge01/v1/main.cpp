...
std::variant<int, const std::string_view>
parse_number(const std::string_view string) {
  std::string val;
...
std::variant<int, const std::string_view>
parse_number_positive(const std::string_view string) {
...
struct FileData {
  ...
  std::optional<std::int16_t> at(const Position &pos) {
    ...
    // std::cout << std::format("Input: {}:{} index: {} / {}", pos.col, pos.row,
    // index, values.size()) << '\n';
...
std::optional<Position> parse_excel_fmt(const std::string_view value) {
  ...
  if (std::holds_alternative<int>(numb)) {
    return {{static_cast<std::size_t>(std::get<int>(numb)),
...
std::optional<Position> parse_comma_fmt(const std::string_view value) {
  ...
  if (std::holds_alternative<int>(varCol) &&
      std::holds_alternative<int>(varRow)) {
    ...
    while (std::getline(ss, temp, ',')) {
      ...
      if (std::holds_alternative<int>(ret)) {
        ...
        if (val < -32768 || val > 32767) {
          error(std::format("Data value to large at: row:{} col:{}", nbrRows,
                            nbrCols));
          return {};
        }
...
void non_interactive_mode(FileData &board,
...
void interactive_mode(FileData &board, const std::filesystem::path &filename) {
  ...
  auto show_help = []() {
    ...
    cout << "   show window -> asks for a range of col and row and will "
            "display it."
         << e;
    cout << "Otherwise: " << e;
    ...
    if (userInput.starts_with("show"sv)) {
      ...
      if (userInput.find("row"sv) != std::string::npos) {
        ...
        if (flr.col - ful.col > 30 || flr.row - ful.row > 30) {
          cout << "Too large of a window to display. Please try a smaller "
                  "window."
               << e;
          continue;
        }
