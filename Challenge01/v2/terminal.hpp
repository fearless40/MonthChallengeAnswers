...
enum class Text { reset = 0, bold = 1, italic = 3, underline = 4 };
...
std::array<const char *, 10> colors{black,   red,  green, yellow,   blue,
                                    magenta, cyan, white, defaultc, reset};
}; // namespace Forecolor
...
namespace detail {
constexpr const char *forecolor_rgb_escape_sequence() { return "\x1b[38;2;"; }

constexpr const char *backcolor_rgb_escape_sequence() { return "\x1b[48;2;"; }

} // namespace detail

constexpr std::string color_rgb_fore_back(int r, int g, int b,
                                          bool fore = true) {
  char buff[32];
  const char *escape = fore ? detail::forecolor_rgb_escape_sequence()
                            : detail::backcolor_rgb_escape_sequence();
  for (int i = 0; i < 8; ++i) {
    buff[i] = escape[i];
  }

  // char buff[32] = fore == true ? detail::forecolor_rgb_escape_sequence() :
  // detail::backcolor_rgb_escape_sequence();
  char *next = &buff[7];
